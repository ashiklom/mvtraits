...
chain2matrix_hier <- function(chain) {
    ...
    colnames(mu_group_mat) <- paste('mu_group', colnames(mu_group_mat), sep = '.')
    sigma_group_mat <- flatten_sigma_group(chain[['Sigma_group']])
    colnames(sigma_group_mat) <- paste('Sigma_group', colnames(sigma_group_mat), sep = '.')
...
flatten_mu_group <- function(mu_group_samples) {
    ...
    mu_group_mat_names <- paste(rep(group_names, nparam), rep(param_names, each = ngroup), sep = '.')
...
flatten_sigma_group <- function(sigma_group_samples) {
    ...
    colnames(group_mat) <- paste(rep(group_names, each = ncolumn), colnames(group_mat), sep = '.')
    return(group_mat)
}
