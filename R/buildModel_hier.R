    ...
    dat_processed <- 
        dplyr::mutate(dat_renested,
                      ...
                      mu_def = dplyr::if_else(npresent == 1,
                                              sprintf('%s = mu_group[%d,%d];', mu_str, group, 
                                                      purrr::map_int(present, 1)),
                                              sprintf('%s = mu_group[%d,%s];', mu_str, group, ind_str)),
                      ...
                      Sigma_def = dplyr::if_else(npresent == 1,
                                                 sprintf('%1$s = Sigma_group[%3$d,%2$d,%2$d];',
                                                         Sigma_str, purrr::map_int(present, 1), group),
                                                 sprintf('%1$s = cholesky_decompose(Sigma_group[%3$d,%2$s,%2$s]);',
