...
test_that("Fit hierarchical normal sequentially", {
  result <- fit_mvnorm_hier(
    dat_full$dat, dat_full$groups, niter = niter, nchains = nchains,
    parallel = FALSE, autofit = autofit, keep_samples = keep_samples
  )
})
...
test_that("Fit hierarchical normal in parallel", {
  result <- fit_mvnorm_hier(
    dat_full$dat, dat_full$groups, niter = niter, nchains = nchains,
    parallel = TRUE, autofit = autofit, keep_samples = keep_samples
  )
})
